...
import {
  OC_PLUGIN_PATH,
  OC_POLICIES_PATH,
  OC_PACKAGE_JSON_PATH,
  OC_PLUGIN_SDK_VERSION,
...
export function generatePluginFiles(
  ...
  // Emitted alongside the plugin so generated tools (which value-import
  // `@opencode-ai/plugin`) and any future runtime-importing files have a
  // declared dependency. Without this file, `cd .opencode && bun install`
  // has nothing to read and opencode load-time fails when resolving
  // `@opencode-ai/plugin` from `.opencode/tools/*.ts`. See
  // docs/opencode-runtime-fixes.md §Finding #3.
  const packageJson = {
    private: true,
    description: "Generated by abax-swarm. Declares the runtime deps required by .opencode/tools/* and .opencode/plugins/*.",
    dependencies: {
      "@opencode-ai/plugin": OC_PLUGIN_SDK_VERSION,
    },
  };
